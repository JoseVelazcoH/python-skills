...
4. **Anti-Patterns** (`python-anti-patterns`): custom import order (all `import x` alphabetical, then stdlib/third-party `from`, blank line, then first-party `from core...`), files under ~300 lines, one cohesive concern per module, no hardcoded literals in logic, domain/config constants out of logic modules and into `constants.py` (or a `constants/<category>.py` subpackage) even if few or single-use, with algorithm-internal sentinels/states kept local as `Enum`s, and no reflexive `from __future__ import annotations` (keep it only when it serves `TYPE_CHECKING`-guarded imports or forward references).
